...
percent.For fitting data was colllected on the uk co2 emissions since 1975
...
df_countries_copy = df_countries[[
    "CO2 emissions/Per Capita", "URBAN POPULATION"]]
...
df_UK_emissions = pd.read_csv("UK_co2emissions.csv")
# plotting the graph
df_UK_emissions.plot("YEAR", "CO2 Emissions")
plt.show()
# finding the inital parameters
param, covar = opt.curve_fit(exponential, df_UK_emissions["YEAR"],
                             df_UK_emissions["CO2 Emissions"])
print("CO2 Emissions", param[0]/1e9)
print("decline rate", param[1])
# plotting the graph with the exponential curve
plt.figure()
plt.plot(df_UK_emissions["YEAR"], exponential(df_UK_emissions["YEAR"],
                                              param[0], param[1]))
plt.plot(df_UK_emissions["YEAR"], df_UK_emissions["CO2 Emissions"])
plt.xlabel("Year")
plt.ylabel("Co2 emissions")
plt.title("Co2 emissions in the UK from 1975-2024")
plt.legend()
plt.show()
